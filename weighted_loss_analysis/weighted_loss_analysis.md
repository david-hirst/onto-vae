...
$$L(\boldsymbol{x}, \theta, \phi) = \lambda \times D_{KL}(q_{\phi}(\boldsymbol{z}|\boldsymbol{x})||p(\boldsymbol{z})) + \sum_d (x_d - \hat{x}_d)^2$$

Underlying this is the assumption of an independent gaussian likelihood for each input node: $p(x_d|\boldsymbol{z}) = N(\hat{x}_d,\sigma^2)$
